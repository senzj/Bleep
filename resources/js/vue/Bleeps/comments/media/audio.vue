...
const props = defineProps({
    ...
    commentId:{ type: [String, Number], default: null },
...
watch(() => props.src, () => {
    if (props.autoInit && props.src) initPlayers();
});
</script>
            ...
            <div class="flex items-center justify-between gap-2">
                ...
                <div class="flex items-center gap-1">
                    <button type="button"
                        ...
                        <span class="play-icon    pointer-events-none"><LucideIcon name="play"  size="20" class="pointer-events-none" /></span>
                        <span class="pause-icon   pointer-events-none" style="display:none;"><LucideIcon name="pause" size="20" class="pointer-events-none" /></span>
                ...
                <div class="flex items-center gap-3 min-w-[70px] justify-end">
                    ...
                    <!-- Volume — horizontal slider popup -->
                    <div class="relative" @click.stop>
                        <!-- Toggle button: always reflects live volume icon -->
                        <button type="button"
                            ...
                            <LucideIcon :name="volumeIcon" size="16" class="pointer-events-none" />
                        </button>
                        ...
                        <Transition
                            ...
                            <div v-if="showVolumeSlider"
                                ...
                                <!-- Mute toggle button — shows volume-x when muted, highlights in error color -->
                                <button type="button"
                                        class="shrink-0 transition-colors"
                                        :class="isMuted ? 'text-error hover:text-error/70' : 'text-base-content/60 hover:text-base-content'"
                                        :aria-label="isMuted ? 'Unmute' : 'Mute'"
                                        @click="toggleMute">
                                    <!-- Always show volume-x when muted, otherwise follow volumeIcon -->
                                    <LucideIcon :name="isMuted ? 'volume-x' : volumeIcon" size="16" class="pointer-events-none" />
                                </button>
                                ...
                                <!-- Percentage label -->
                                <span class="text-xs text-base-content/60 w-8 text-right tabular-nums">
                                    {{ volume }}%
                                </span>
                            </div>
                        </Transition>
